...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("searchButton").addEventListener("click", () => {
    console.log(3,searchColor);
    fetchClothingImages(searchColor);
  });

  // Function to fetch clothing images based on selected colors
  async function fetchClothingImages(color) {
    let colorm=document.getElementById("selectedColors").
    console.log(colorm);
    console
    const req = await fetch(
      "http://localhost/omogene/php/selectClothing.php?color=" + colorm
    ...
    const box = document.getElementById("clothingImages");

    let div = "";
    for (let el in res.data) {
      console.log(res.data[el].clothing_img);
      div += `<div><span>${res.data[el].id}</span> => <span>${res.data[el].clothing_catigory}</span> <img src="http://localhost/projects/s/${res.data[el].clothing_img}"/></div>`;
    }
    box.innerHTML = div;
  }
});
