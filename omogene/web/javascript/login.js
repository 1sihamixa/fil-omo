...
signSubmit.addEventListener('click', function () {
  ...
  let signConfirmPassword = document.getElementById('signConfirmPassword').value;

  let request = new XMLHttpRequest();
  request.open('POST', "signup.php", true);
  request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
  request.send('username=' + signUsername + '&email=' + signEmail + '&password=' + signPassword + '&cpassword=' + signConfirmPassword);
